% Create accession-specific Arabidopsis PCM versions
...
listOfAccessions = cell.empty; % if empty, will use all accession with BLAST results
blastDir = 'Data/analysis/accSpecPCM/03_BLAST_files/';  % relative to projDir
modelOutPath = 'Data/analysis/accSpecPCM/04_models/';  % relative to projDir
...
if isempty(listOfAccessions)
    ...
    for i = 1:length(fileList)
        ...
        orgs{i} = baseName(1:end-length('.out'));
    end
end

%% make org specific: replace gprs, find missing reactions
for orgIx = 1:length(orgs)
    org = orgs{orgIx};
    ...
    while ~solved && ~broken
        ...
        [y_res, v_res] = carvePCM(orgPcm, M, eps, ...
            ...
            rxn_scores, media, true, true, FeasibilityTol);
        ...
        if sol.f >= min(min_gr_photo * 0.8, solInit.f * 0.8)
            solved = true;
        elseif FeasibilityTol >= 1e-8
            FeasibilityTol = 1e-9;
            eps = 1e-8;
            disp(['Solution not found, reducing FeasibilityTol to ' ...
                num2str(FeasibilityTol)]);
        else
    ...
    writeCbModel(carved, 'fileName', [modelOutPath, org, '.mat']);
    writeCbModel(carved, 'fileName', [modelOutPath, org, '.xml']);
end

%% main function
function [orgPcm, missingInOrg] = makeOrgSpecific(pcm, blastFilePath, ...
            requireAllSubunits)
    ...
    % read BLAST results
    blastResults = readtable(blastFilePath, 'FileType', 'text', ...
        'Delimiter', '\t');
    ...
    % Initialize an array to hold the species-specific genes
    orgRxnGenes = cell(length(pcm.rxns), 1);
    
    % Initialize an array to track reactions missing genes in the species
    ...
    for i = 1:length(orgPcm.rxns)
        ...
        % associate org genes to union genes in this step) and non-enzymatic rxns
        ...
        orgRxnGenes{i} = unique(currRxnOrgGenes);
    end
    ...
    for i = 1:length(orgPcm.rxns)
        ...
        elseif ~contains(currentUnionGpr, ' AND ', 'IgnoreCase', true)
            ...
            % in case of AND, a one-to-one mapping of union genes to species 
            % genes is required.
    
            % these two arrays will hold the union genes associated to
